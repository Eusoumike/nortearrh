...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: ticket, error: tErr } = await admin
      .from("tickets")
      .select(
        `*, clients!fk_tickets_client (organization, razao_social, company, name, cnpj, contact_name, contact_email, contact_phone, phone, email), ticket_interactions!fk_interactions_ticket (type, summary, content, created_at)`,
      )
      .eq("id", ticket_id)
      .single();
